...
Die Verfahren, um die in diesem Kapitel beschriebenen Größen der Kodierungen zu berechnen, wurden in Python implementiert. Die Messung der Zeit für die Rekonstruktion der Teilalignments erfolgte aufgrund der effizienz hinsichtlich der Geschwindigkeit mit der C-Implementierung. Für die Messungen wurde ein MacBook Pro 11.1 mit einem 2.4 GHz Intel Core i5 Prozessor und 4 GB Arbeitsspeicher verwendet.
...
Um die Zeit, die für die Rekonstruktion der Teilalignments in Abhängigkeit von $\Delta$ benötigt wird, berechnen zu können, wurden für $\Delta$-Werte im Bereich von $5$ bis $500$ jeweils $50$ Sequenzpaare der Länge \numprint{5000} mit einer Fehlerrate von $15$\% wie oben beschrieben berechnet und die Trace Point Differenzen berechnet, anschließend dekodiert und die Zeit dafür gemessen. Diese Dekodierung entspricht der Rekonstruktion der Teilalignments und der anschließenden Konkatenation zu einem Gesamtalignment. Die Resultate sind in Tabelle \ref{time-table} beschrieben und in Abbildung \ref{time} numerisch dargestellt.
...
\begin{figure}[t]
	\centering
	\includegraphics[width=13cm]{coding/time}
...
\begin{table}[t]
	...
	\begin{tabular}{|c|c|}
		...
		\hline
	\end{tabular}
	\caption{Zeitbedarf der Rekonstruktion der Teilalignments in Abhängigkeit des $\Delta$-Wertes}
	\label{time-table}
\end{table}
...
Die Abbildung \ref{time} verdeutlicht diese Abhängigkeit. Für ein $\Delta$ zwischen $5$ und $200$ werden demnach durchschnittlich etwa $0.000276$ Sekunden für die Rekonstruktion benötigt. Für $\Delta > 200$ wächst die Zeit für linear an und erreicht für ein $\Delta = 414$ mit $0.000379$ Sekunden, also etwa $27$\% mehr als für ein $\Delta \leq 200$, das Maximum. Mit Hilfe von $\Delta$ kann somit die Zeit, die für die Rekonstruktion der Teilalignments benötigt wird, eingestellt werden.
