...
Ziel dieser Bachelorarbeit ist es, verschiedene Repräsentationen von paarweisen Sequenzalignments und deren Kodierungen zu beschreiben und zu vergleichen, sowie basierend auf der eigenen Implementierung einer speichereffizienten Repräsentation Unterschiede zu diskutieren.

Die verschiedenen Operationen, die Symbole der einen Sequenz in die andere zu überführen, können je nach Verfahren unterschiedlich dargestellt und gewichtet werden. Bei Gleichheit wird sie als 'match', bei einer Substitution als 'mismatch', bei einer Löschung als 'deletion' und bei einer Einfügung als 'insertion' dargestellt.

Um die verschiedenen Sequenzen vergleichen zu können, berechnet man für das Alignment einen Score oder die Kosten, um den Aufwand beschreiben zu können, die gegebenene Sequenz in die Zielsequenz umzuwandeln. Hierbei wird jeweils das Optimum, also entweder der maximale Score oder die minimalen Kosten gesucht. Ähnliche Sequenzen haben einen hohen Score und geringe Kosten und unterschiedliche Sequenzen analog einen kleinen Score und hohe Kosten.
...
Wenn die Edit-Distanz $e$ mit der Einheitskostenfunktion bekannt ist, kann ein optimales lokales Alignment in $O(e^2)$ Zeit berechnet werden \cite{myers}.
...
Insgesamt benötigt die naive binäre Kodierung also $19 \cdot \lceil \log_26\rceil + \lceil \log_23\rceil + 18 = 77$ Bit für die Kodierung des CIGAR-Strings.
...
Das Verfahren bietet den Vorteil, durch die Größe $\Delta$ der Teilabschnitte die Anzahl der Trace Points und somit den Speicherbedarf und die Zeit, die für die Berechnung der Teilalignments benötigt wird, anpassen zu können.
...
Seien $i,j,k,\ell,\Delta$ und die Trace-Points eines Alignments von $u$ und $v$ gegeben. Damit kann ein Alignment $A'$ von $u$ und $v$ mit $\delta (A') \leq \delta (A)$ konstruiert werden. Aus den Trace-Points werden zunächst die Substring-Paare $u_q$ und $v_q$ für alle $q, 0 \leq q \leq \tau - 1$ bestimmt. Hierfür wird jeweils ein optimales Alignment berechnet und anschließend die Alignments von den aufeinanderfolgenden Substring-Paaren zu $A'$ konkateniert.
...
Bei gleichmäßig ansteigenden Werten ist die Abweichung der Differenzen zweier aufeinanderfolgender Werte in der Liste untereinander gering. Dadurch ist auch die Menge der zu kodierenden Symbole klein.

Im folgenden Beispiel wird die Kodierung der Trace Point Differenzen ausführlich für die naive binäre, unäre und Huffman-Kodierung erläutert.
...
Für die Kodierung der Trace Points ist in diesem Beispiel eine Differenzen-Kodierung sinnvoll, da $L$ die oben genannten Eigenschaften einer gleichmäßig ansteigenden Liste erfüllt und somit die Menge der zu kodierender Symbole klein ist.

Daraus ergibt sich $L_{\mathit{diff}} = (5, 5, 5, 5, 4, 4, 6)$.

Um aus den Trace Points ein neues Alignment rekonstruieren zu können, benötigt man zusätzlich den $\Delta$-Wert, sowie die Start- und End-Positionen der Sequenzen die aligniert werden, damit die Grenzen der Substrings beider Sequenzen berechnet werden können. Hierfür kann der $\Delta$-Wert zu $L_{\mathit{diff}}$ hinzugefügt werden, da er bei $\Delta$ großen Teilabschnitten in $u$ wahrscheinlich der dominierende Wert in $L_{\mathit{diff}}$ sein wird.
...
Bei der naiven binären Kodierung von $L_{\mathit{diff}}$ ergibt sich analog zu der Darstellung in Kapitel \ref{Cigar_Speicher} ein Bedarf von $\lceil \log_28\rceil = 3$ Bit pro Symbol, also $8 \cdot 3 = 24$ Bit insgesamt und damit nur  $\frac{24}{77} = 31.17$\% der Größe der naiven binären Kodierung für den CIGAR-String.

Für die unäre Kodierung ergibt sich für dieses Beispiel die in Tabelle \ref{diff-unary} beschriebene Kodierung. Die Größe dieser Kodierung ist demnach $12$ Bit und benötigt damit nur $\frac{12}{67} = 17.91$\% der unären Kodierung für den CIGAR-String.
...
Für den in Abschnitt \ref{Cigar_Speicher} genannten CIGAR-String $S$ mit der Häufigkeitsverteilung aus Tabelle \ref{cig-huf-coding} ergibt sich eine Entropie von
...
Die in Abschnitt \ref{Differenzen-Kodierung} genannten Trace Point Differenzen des selben Alignments inklusive des $\Delta$-Wertes ergeben analog
...
Zusammenfassend werden die sich daraus ergebenden Größen der Kodierungen des Alignments aus Beispiel \ref{Bsp1} in Tabelle \ref{Kodierung-Tabelle} dargestellt.
\begin{table}[t]
	\centering
	\begin{tabular}{|c|c|c|}
		\hline
...
Diese Funktion unterteilt die Sequenzen wie in Kapitel \ref{TP Konzept} beschrieben und bestimmt die Trace Points, welche dann ebenfalls in der Trace Point Liste gespeichert werden. Der Pseudocode der encode-Funktion ist in Abbildung \ref{encode} beschrieben. Hierbei geht die Information, wie die jeweiligen Intervalle zwischen den Trace Points zu den komplementären Intervallen in der Ursprungssequenz aligniert werden, verloren. Es sind also lediglich die zu alignierenden Abschnitte bekannt, aber nicht die Alignierung selbst. Diese muss bei der Dekodierung neu berechnet werden.

Die Dekodierung der Trace Points erfolgt in der decode-Funktion, welche ebenfalls im TracePoint-Modul definiert ist. Sie nimmt eine Trace Point Liste als Parameter entgegen und berechnet für jedes Paar von zu alignierenden Teilabschnitten eine neue Liste von Edit-Operationen und konkateniert diese abschließend zu einer Gesamtliste und gibt sie als Rückgabewert zurück. Der Pseudocode der decode-Funktion ist in Abbildung \ref{decode} zu finden. Hierbei ist nicht gewährleistet, dass das neue Alignment dem alten entspricht. Das neue Alignment hat jedoch, wie in Abschnitt \ref{TP Konzept} beschrieben, keine höheren Kosten als das Ausgangsalignment.
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		...
		\EndIf
		\If{$uChars = uTP[count] + 1$ \textbf{and} $count < \tau - 1$}
		...
		\EndFunction
	\end{algorithmic}
	\captionof{figure}{Pseudocode des Algorithmus zur Berechnung der Trace Points aus einer gegebenen Liste von Edit-Operationen}
	\label{encode}
\end{algorithm}
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		...
		\EndFunction
	\end{algorithmic}
	\captionof{figure}{Pseudocode des Algorithmus zur Berechnung einer Liste von Edit-Operationen aus einer gegebenen Trace Point Liste}
	\label{decode}
\end{algorithm}
...
Die gemessenen Werte der Größe der Kodierung der CIGAR-Strings sind in Tabelle \ref{bits-result} enthalten. Die Häufigkeitsverteilung der Kodierung für \numprint{1000} CIGAR-Strings mit je \numprint{5000} Basenpaaren und einer Fehlerrate von $15$\% ist in Abbildung \ref{cigar-coding} grafisch dargestellt.
...
\begin{figure}[t]
	...
	\caption{Mittelwerte der Huffman-Kodierung der Trace Point Differenzen für \numprint{5000} Basenpaare, einer Fehlerrate von $15$\% und $\Delta$-Werten von $5$ bis $500$.} \label{var-deltas}
\end{figure}

\begin{figure}[t]
	\centering
	\includegraphics[width=13cm]{images/data/err-rate}
	\caption{Mittelwerte der Huffman-Kodierung der Trace Point Differenzen für \numprint{5000} Basenpaare, einem $\Delta$-Wert von $100$ und Fehlerraten von $5$\% bis $35$\%.} \label{var-errs}
\end{figure}
...
Für die Rekonstruktion der Teilalignments wird in Abhängigkeit von $\Delta$ viel Zeit benötigt, da $\Delta$ die Größe der Teilabschnitte bestimmt und die optimalen Teilalignments, wie in Kapitel \ref{methoden} beschrieben, in $O(e^2)$ Zeit mit $e$ als Edit-Distanz rekonstruiert werden können.
...
Für Sequenzen der Länge \numprint{5000} mit $\varepsilon = 15\%$ erlaubt die durch Regression berechnete Funktion $f(\Delta) = -\frac{1}{50}\Delta + 93$ eine Abschätzung des Speicherbedarfs und die Funktion $f(\Delta) = 0.28\Delta - 1.23$ eine Abschätzung der Zeit in Abhängigkeit von $\Delta$.
